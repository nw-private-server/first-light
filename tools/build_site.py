    ...
    return [
        ...
        },
        {
            ...
            "summary": "Wake 8 identified a single writer "
                       "(`FUN_140fb3560:452`) that sets a destroy "
                       ...
                       "`local=0xFE476177; "
                       "(*vtable+0x608)(arg, DAT_147efa330)`, and "
                       "received by 5 subscribers each writing a "
                       "distinct Carrier-state flag: `[+0xcd]` "
                       "(`FUN_1471f15d0`), `[+0xcf]` "
                       "(`FUN_146074880`), `[+0xda]` "
                       "(`FUN_146b621c0`), `[+0xfd]` "
                       "(`FUN_140fb3560` — the wake-8 destroy "
                       "flag, one subscriber response among many), "
                       "and `[+0x179]` (`FUN_1402a6310`). The "
                       "Carrier state structure has at least 7 "
                       "lifecycle-related flags (including `[+0xd9]` "
                       "and `[+0xfc]` written by adjacent events) "
                       "in the byte range `[+0xcd]` → `[+0x179]`. "
                       "**The event family** dispatched alongside "
                       "`0xFE476177` totals ~50 related `AZ::Crc32` "
                       "hashes — `0xF2D0BB74`, `0x53E4E683`, "
                       "`0xDECE4567`, `0xAD273586`, `0x8E281F3D`, "
                       "plus shared sub-action hashes and "
                       "namespace IDs. **Implication for the "
                       "destroy-loop framing**: the destroy chain "
                       "is one subscriber's response to a broader "
                       "broadcast — not the event's primary "
                       "purpose. The event is likely `OnDisconnect` "
                       "/ `OnConnectionLost` / similar Carrier "
                       "lifecycle broadcast. Resolving the family "
                       "name now has 50× the surface area of the "
                       "wake-9 attempt: a single hash match (via "
                       "O3DE-corpus brute-force using "
                       "`analysis/crc32_FE476177_brute_force.py`, "
                       "OR runtime Frida hook on any emitter or "
                       "subscriber) identifies the EBus and "
                       "constrains the other 49 hashes to the "
                       "same event class.",
        },
        {
