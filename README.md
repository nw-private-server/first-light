...
3. **Runtime trace on `FUN_140fb3560` to identify the destroy-trigger event name.** The destroy-loop trigger writer was identified as `FUN_140fb3560:452` (wake 8) — `[R13+0xfd]` is a skip-timeout-and-flush flag on the GridMate Carrier, set only when an event with id `0xFE476177` fires. That event id is an `AZ::Crc32` hash, but the release build has the source string stripped (29 references to the constant, 0 adjacent string literals). A Frida hook logging the event-id argument structure on `FUN_140fb3560` would identify the event name; static-RE is exhausted on this branch (see `analysis/state_machine_summary.md` § A3.1).
