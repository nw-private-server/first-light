...
| A3.1 | What does `0xFE476177` hash to? | partial — string stripped, runtime needed. Brute-force attempts at wakes 9 / 277 / 278 / 279 / 280 exhausted hand-curated wordlists (~1300+ CRC computations). AzCore CRC32 algorithm verified to match zlib (polynomial 0xEDB88320). **Substantial new structural finding (wakes 278-280)**: 0xFE476177 is fired from at least 4 emitter sites (FUN_1402af830, FUN_140fb84b0, FUN_146b64550, FUN_1471f4260 — all using pattern `local=0xFE476177; vtable+0x608(arg, DAT_147efa330)`), and received by at least 5 subscribers each writing a distinct Carrier-state flag: `[+0xcd]` (FUN_1471f15d0), `[+0xcf]` (FUN_146074880), `[+0xd9]` (FUN_146b621c0 — different event), `[+0xda]` (FUN_146b621c0 for 0xFE476177), `[+0xfd]` (FUN_140fb3560 — the wake-8 destroy flag), `[+0x179]` (FUN_1402a6310). This is a broadcast lifecycle event in the Carrier system with ~50 hashes total in its event family. The destroy is one subscriber response, not the event's purpose. Likely an OnDisconnect / OnConnectionLost / similar lifecycle event. See `analysis/crc32_FE476177_brute_force.py` for the 50-hash target list. |
